...
namespace ServerTool
{
    class Licenses
    {
        ...
        {
            uint lpflOldProtect;
            VirtualProtect(address, address.Length, PageExecuteReadwrite, out lpflOldProtect);
        }

        private static bool nowEnglish = false;

        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern bool VirtualProtect(byte[] lpAddress, int dwSize, uint flNewProtect,
        ...
        private const int PageExecuteReadwrite = 64;

        private static readonly byte[] buf_asm =
        {
            85, 139, 236, 129, 236, 192, 0, 0, 0, 83, 86, 87, 141, 189, 64, 255, 255, 255, 185, 48, 0, 0, 0, 184, 204,
            204, 204, 204, 243, 171, 184, 0, 0, 0, 0, 51, 210, 15, 162, 137, 85, 252, 137, 69, 248, 184, 1, 0, 0, 0, 51,
            201, 51, 210, 15, 162, 137, 85, 244, 137, 69, 240, 139, 69, 252, 137, 69, 236, 139, 69, 248, 137, 69, 232,
            139, 69, 244, 137, 69, 228, 139, 69, 240, 137, 69, 224, 141, 69, 236, 95, 94, 91, 139, 229, 93, 195
        };

        public static string GetDays(TimeSpan time)
        {
            if (nowEnglish == true)
            {
                return ((time < TimeSpan.Zero)
                    ? String.Format("License expired {0} days {1} hours {2} minuts before",
                        Math.Abs(time.Days).ToString(), Math.Abs(time.Hours).ToString(),
                        Math.Abs(time.Minutes).ToString())
                    : String.Format("License active. Left {0} days {1} hours {2} minutes", time.Days, time.Hours,
                        time.Minutes));
            }
            else
                return ((time < TimeSpan.Zero)
                    ? String.Format("Лицензия истекла {0} дней {1} часов {2} минут назад",
                        Math.Abs(time.Days).ToString(), Math.Abs(time.Hours).ToString(),
                        Math.Abs(time.Minutes).ToString())
                    : String.Format("Лицензия активна. Осталось {0} дней {1} часов {2} минут", time.Days, time.Hours,
                        time.Minutes));
        }
        ...
        {
            string tokenString = String.Format("token={0}&hwid={1}",
                DigitalSign.EncryptString(token1.ToString(), preKey), GetCPUID());
            return tokenString;
        }
        ...
        public static string CheckLic()
        {
            ...
            string urlToScript =
                DigitalSign.XOR("https://connect.dayz-tool.online/projects/rsa_generator/base.php", XORkey);
            ...
            string responseXORed =
                DigitalSign.XOR(
                    GetRequest(DigitalSign.XOR(urlToScript, XORkey), DigitalSign.XOR(infoXORed, XORkey)),
                    XORkey);
            string[] responseSplitted = DigitalSign.XOR(responseXORed, XORkey)
            ...
            try
            {
                if (responseSplitted[0].Split('=')[1] == "1")
                {
                    ...
                    {
                        if (GetCPUID() == Encoding.UTF8.GetString(Convert.FromBase64String(responseSplitted[1]
                            .Split(new char[] {'[', ']'}, StringSplitOptions.RemoveEmptyEntries)[0])))
                        {
                            if (DigitalSign.CompareRSAMethod(GetSignedText(responseSplitted),
                                responseSplitted[responseSplitted.Length - 1].Split(new char[] {'[', ']'},
                                    StringSplitOptions.RemoveEmptyEntries)[0], DigitalSign.XOR(pubKey, XORkey)))
                            {
                                ...
                                return GetDays(ActivatedTime);

                            }
                            else
                            {
                                return "Лицензия не приобреталась";
                            }
                        }
                        else
                        {
